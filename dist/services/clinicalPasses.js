...
exports.buildTranscriptRepairPrompt = buildTranscriptRepairPrompt;
exports.applyRepairs = applyRepairs;
exports.buildCompletenessPrompt = buildCompletenessPrompt;
exports.measureNoteDepth = measureNoteDepth;
const clinicalLexicon_1 = require("./clinicalLexicon");
function buildTranscriptRepairPrompt(segments) {
    const vocabulary = clinicalLexicon_1.CLINICAL_PHRASES.join(', ');
...
TRANSCRIPT:
${JSON.stringify(segments.map((s) => ({ segmentId: s.id, speaker: s.mappedRole ?? 'UNATTRIBUTED', text: s.text })), null, 2)}`;
}
 ...
 */
function applyRepairs(segments, corrections) {
...
function buildCompletenessPrompt(draftNote, segments) {
 ...
 */
function measureNoteDepth(note) {
    ...
    const walk = (node) => {
        if (Array.isArray(node)) {
            if (node.length > 0 && node[0] && typeof node[0].value === 'string') {
                const real = node.filter((c) => c.value && !c.value.includes('Not documented') && c.value !== 'Not stated');
                if (real.length > 0) {
                    populatedFields++;
                    totalClaims += real.length;
                    totalWords += real.reduce((n, c) => n + c.value.trim().split(/\s+/).filter(Boolean).length, 0);
                }
                else {
                    emptyFields++;
                }
        ...
        if (node && typeof node === 'object') {
            for (const key of Object.keys(node)) {
                if (key === 'warnings' || key === 'clinicianReviewFlags')
                    continue;
                walk(node[key]);
            }
        }
