...
exports.CLINICAL_PHRASES = void 0;
exports.normaliseMeasurements = normaliseMeasurements;
exports.correctClinicalText = correctClinicalText;
exports.clinicalScore = clinicalScore;
exports.rescoreAlternatives = rescoreAlternatives;
exports.getRecognitionPhraseHints = getRecognitionPhraseHints;
exports.buildJsgfGrammar = buildJsgfGrammar;
/** Canonical clinical vocabulary: recognition phrase-bias list and rescoring target set. */
exports.CLINICAL_PHRASES = [
 ...
 */
function normaliseMeasurements(text) {
    const before = text;
    let out = text;
    ...
    out = out.replace(new RegExp(`\\b(${tens})[\\s-](${ones})\\s+(${UNIT_ALT})\\b`, 'gi'), (_m, t, o, u) => `${NUMBER_WORDS[t.toLowerCase()] + NUMBER_WORDS[o.toLowerCase()]} ${u}`);
    out = out.replace(new RegExp(`\\b(${tens}|${under20})\\s+(${UNIT_ALT})\\b`, 'gi'), (_m, n, u) => `${NUMBER_WORDS[n.toLowerCase()]} ${u}`);
 ...
 */
function correctClinicalText(rawText) {
    const corrections = [];
    let text = rawText;
...
// ---------------------------------------------------------------------------
const PHRASE_TOKENS = new Set(exports.CLINICAL_PHRASES.flatMap((p) => p.toLowerCase().split(/[\s-]+/)).filter((w) => w.length > 3));
/** How "clinical" a candidate transcription is. Higher is more likely correct. */
function clinicalScore(candidate) {
    const lower = candidate.toLowerCase();
    let score = 0;
    for (const phrase of exports.CLINICAL_PHRASES) {
        if (lower.includes(phrase.toLowerCase()))
            score += 3;
    }
    for (const token of lower.split(/[^a-z0-9']+/)) {
        if (token.length > 3 && PHRASE_TOKENS.has(token))
            score += 1;
    }
    const measurements = lower.match(new RegExp(`\\d+(\\.\\d+)?\\s*(${UNIT_ALT})\\b`, 'g'));
    if (measurements)
        score += measurements.length * 2;
    return score;
}
/**
 ...
 */
function rescoreAlternatives(alternatives) {
    const cleaned = alternatives.filter((a) => a && typeof a.transcript === 'string' && a.transcript.trim().length > 0);
    if (cleaned.length === 0) {
        return {
            transcript: '',
            confidence: null,
    ...
    return {
        ...
        confidence: typeof best.confidence === 'number' && best.confidence > 0 ? best.confidence : null,
        promoted: best !== top,
 ...
 */
function getRecognitionPhraseHints() {
    return [...exports.CLINICAL_PHRASES];
}
/** JSGF grammar for the W3C SpeechGrammarList, where the browser supports it. */
function buildJsgfGrammar() {
    const escaped = exports.CLINICAL_PHRASES.map((p) => p.replace(/[;|=<>*+()[\]{}]/g, '')).join(' | ');
