...
class VTTParserService {
    parseVTT(vttContent, meetingId) {
        ...
        let currentTimestampMatch = null;
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (line && !line.match(/^\d+$/) && currentTimestampMatch) {
                ...
                const isDuplicate = segments.some((s) => s.startTimeMs === currentTimestampMatch.startMs && s.text === text);
                if (isDuplicate) {
                    ...
                    segments.push({
                        ...
                        mappedRole: currentSpeaker.toLowerCase().includes('therapist') ? client_1.ParticipantRole.THERAPIST : client_1.ParticipantRole.CLIENT,
                        text,
                        confidence: 1.0,
                        overlapStatus: client_1.OverlapStatus.CLEAR,
