...
exports.logRequestDiagnostics = logRequestDiagnostics;
/**
...
function describeShape(body, depth = 0) {
    if (depth > 2 || body === null || typeof body !== 'object')
        return [];
    const out = [];
...
function logRequestDiagnostics(req, label) {
    ...
    return {
        finish(status, extra = {}) {
            ...
            const shape = status >= 400 && status < 500 ? ` fields=[${describeShape(req.body).join(', ')}]` : '';
            const extras = Object.entries(extra)
                .map(([k, v]) => `${k}=${Array.isArray(v) ? `[${v.join('|')}]` : String(v)}`)
                .join(' ');
            console.log(`[req] ${label} status=${status} ms=${Date.now() - started} ` +
