...
exports.GeminiModelClient = void 0;
exports.getClinicalModelClient = getClinicalModelClient;
exports.setClinicalModelClient = setClinicalModelClient;
...
class GeminiModelClient {
    ...
    async generate(systemInstruction, userContent) {
        const apiKey = env_1.env.LLM_API_KEY || process.env.GEMINI_API_KEY;
        if (!apiKey) {
            throw new Error('The clinical language model is not configured on this server (no Gemini API key). ' +
                'The transcript has been saved; documentation can be generated once it is configured.');
        }
        if (!this.resolvedModelInfo) {
            this.resolvedModelInfo = await (0, GeminiLLMProvider_1.resolveAvailableModel)(apiKey, env_1.env.GEMINI_MODEL);
        }
        console.log(`[gemini] Executing generateContent using model="${this.resolvedModelInfo.modelName}" ` +
            `(method: ${this.resolvedModelInfo.selectionMethod}, explicit: ${this.resolvedModelInfo.isExplicit}).`);
        let genAI = new generative_ai_1.GoogleGenerativeAI(apiKey);
        ...
        try {
            ...
            if (is404 && !this.resolvedModelInfo.isExplicit) {
                console.warn(`[gemini] Model "${this.resolvedModelInfo.modelName}" returned 404 during generation. Invalidating cache and re-discovering.`);
                const rejected = new Set([this.resolvedModelInfo.modelName]);
                (0, GeminiLLMProvider_1.resetModelCache)();
                this.resolvedModelInfo = await (0, GeminiLLMProvider_1.resolveAvailableModel)(apiKey, env_1.env.GEMINI_MODEL, true, rejected);
                if (this.resolvedModelInfo) {
     ...
     */
    async callWithRetries(model, userContent, maxAttempts = 3) {
        let lastError;
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                if (blockReason) {
                    throw new Error(`The clinical model declined to process this transcript (${blockReason}). This can ` +
                ...
                console.warn(`[gemini] Provider error (attempt ${attempt}/${maxAttempts}): ` +
                    `HTTP ${status || 'N/A'}, category=${isAuth
                        ? 'AUTH'
                        : isRateLimited
                            ? 'RATE_LIMIT'
                            : isNotFound
                                ...
                                : isTransient
                                    ? 'PROVIDER_5XX'
                                    : 'OTHER'}, message="${message.slice(0, 150)}"`);
                if (/declined to process this transcript/.test(message))
                    throw err;
                if (isAuth) {
                    throw new Error(`Gemini API authentication or permission failed (HTTP ${status || 403}). Check GEMINI_API_KEY environment variable.`);
                }
                if (isNotFound) {
                    if (this.resolvedModelInfo?.isExplicit) {
                        throw new Error(`Configured GEMINI_MODEL "${this.resolvedModelInfo.modelName}" is not available to this API key (HTTP 404). Update or unset GEMINI_MODEL.`);
                    }
                    throw new Error('Clinical documentation could not be generated because the AI service is temporarily unavailable. ' +
                        'Your transcript has been saved — please retry.');
                }
        ...
        if (/rate limit|quota|RESOURCE_EXHAUSTED|429/i.test(message)) {
            throw new Error('The clinical model is rate limited on the current API quota. The transcript has been ' +
...
exports.GeminiModelClient = GeminiModelClient;
let cached = null;
function getClinicalModelClient() {
    if (cached)
        return cached;
    if (env_1.env.LLM_PROVIDER === 'openrouter') {
        const apiKey = env_1.env.OPENROUTER_API_KEY || env_1.env.LLM_API_KEY || process.env.OPENROUTER_API_KEY;
        if (!apiKey) {
            throw new Error('The clinical language model is not configured on this server (missing OPENROUTER_API_KEY). ' +
                'The transcript has been saved; documentation can be generated once it is configured.');
        }
        const modelName = env_1.env.OPENROUTER_MODEL || 'google/gemini-2.0-flash-exp';
        cached = new OpenRouterLLMProvider_1.OpenRouterModelClient(apiKey, modelName);
        return cached;
    }
...
/** Test seam. */
function setClinicalModelClient(client) {
    cached = client;
}
