...
exports.testMinimalModelGeneration = testMinimalModelGeneration;
exports.resolveAvailableModel = resolveAvailableModel;
exports.resetModelCache = resetModelCache;
exports.mergeModelNote = mergeModelNote;
exports.groundClaims = groundClaims;
exports.unwrapRoot = unwrapRoot;
exports.transcriptForModel = transcriptForModel;
...
async function testMinimalModelGeneration(apiKey, modelName) {
    try {
        ...
        console.warn(`[gemini-probe] Candidate "${modelName}" probe failed: status=${status || 'N/A'}, category=${errorCategory}, message="${msg.slice(0, 150)}"`);
 ...
 */
async function resolveAvailableModel(apiKey, configuredModel, forceRefresh = false, rejectedInRun = new Set()) {
    if (resolvedModelCache && !forceRefresh)
 ...
 */
function mergeModelNote(skeleton, model) {
    ...
    if (typeof skeleton === 'object' && skeleton !== null) {
        ...
        for (const key of Object.keys(skeleton)) {
            if (key in model)
                out[key] = mergeModelNote(skeleton[key], model[key]);
        }
        return out;
    }
 ...
 */
function groundClaims(node, segmentMap, dropped) {
    if (Array.isArray(node)) {
        if (node.length > 0 && isClaim(node[0])) {
            const kept = node.filter((claim) => {
                if (claim.value === NOT_DOCUMENTED || claim.sourceClassification === 'NOT_STATED')
                    return true;
                const evidence = Array.isArray(claim.evidence) ? claim.evidence : [];
                const valid = evidence.filter((ev) => ev && segmentMap.has(ev.segmentId));
                if (valid.length === 0) {
 ...
 */
function transcriptForModel(segments) {
    return segments.map((s) => ({
        ...
        text: s.text,
        transcriptionConfidence: s.confidence === null || s.confidence === undefined ? 'UNKNOWN' : s.confidence,
...
class GeminiLLMProvider {
    ...
    async checkHealth() {
        ...
        try {
            ...
            const activeInfo = await resolveAvailableModel(apiKey, env_1.env.GEMINI_MODEL);
            return {
                status: 'CONNECTED',
                providerName: this.name,
     ...
     */
    async repairTranscript(model, segments, notes) {
        try {
            const result = await model.generateContent((0, clinicalPasses_1.buildTranscriptRepairPrompt)(segments));
            ...
            if (refused.length > 0) {
                notes.push(`${refused.length} proposed transcript correction(s) were rejected for altering a ` +
     ...
     */
    async deepenNote(model, note, segments, segmentMap, dropped) {
        const before = (0, clinicalPasses_1.measureNoteDepth)(note);
        try {
            const result = await model.generateContent((0, clinicalPasses_1.buildCompletenessPrompt)(note, segments));
            const parsed = JSON.parse(stripCodeFence(result.response.text()));
            ...
            if (after.totalWords > before.totalWords) {
                console.log(`[GeminiLLMProvider] Completeness pass deepened the note: ` +
    ...
    async extractStructuredNote(segments) {
        if (!segments || segments.length === 0) {
            throw new Error('[GeminiLLMProvider] No transcript segments supplied. A clinical note cannot be ' +
        ...
        const modelInfo = await resolveAvailableModel(apiKey, env_1.env.GEMINI_MODEL);
        ...
        const prompt = `${(0, aiExtraction_1.generateSystemPrompt)()}\n\n` +
            ...
            try {
                ...
                if (dropped.length > 0) {
                    ...
                    grounded.clinicianReviewFlags = [
                        ...
                        {
                            flagType: 'OTHER',
                            description: `Ungrounded statements removed before review: ${dropped.slice(0, 5).join(' | ')}` +
        ...
        console.error('[GeminiLLMProvider] All attempts failed; returning the deterministic fallback draft, ' +
