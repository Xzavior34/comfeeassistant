...
exports.buildSpeechContexts = buildSpeechContexts;
exports.groupWordsBySpeaker = groupWordsBySpeaker;
 ...
 */
function buildSpeechContexts(extraPhrases = []) {
    const clinical = (0, clinicalLexicon_1.getRecognitionPhraseHints)();
 ...
 */
function groupWordsBySpeaker(words, maxPauseMs = 1500) {
    const segments = [];
    let current = null;
    const flush = () => {
...
class GoogleSpeechProvider {
    ...
    async checkHealth() {
        ...
        if (!hasCredential) {
            return {
                ...
                details: 'Set GOOGLE_SPEECH_API_KEY (API key) or GOOGLE_ACCESS_TOKEN (OAuth bearer token) to ' +
        ...
        return {
            status: 'CONNECTED',
            providerName: this.name,
            details: `Speaker diarisation enabled. Clinical speech adaptation active ` +
                `(${(0, clinicalLexicon_1.getRecognitionPhraseHints)().length} boosted phrases). Language: ${process.env.SPEECH_LANGUAGE || 'en-GB'}.`
        };
    }
    authQuery() {
    ...
    async awaitOperation(operation, timeoutMs = 15 * 60 * 1000) {
        if (operation.done) {
            if (operation.error) {
        ...
        if (!name)
            throw new Error('[GoogleSpeechProvider] Operation returned without a name.');
        const started = Date.now();
        let delay = 2000;
        while (Date.now() - started < timeoutMs) {
            await new Promise((r) => setTimeout(r, delay));
            delay = Math.min(delay * 1.5, 15000);
            const poll = await fetch(`https://speech.googleapis.com/v1/operations/${encodeURIComponent(name)}${this.authQuery()}`, { headers: this.authHeaders() });
            if (!poll.ok)
                continue;
    ...
    async resolveAudio(audioUri) {
        ...
        if (approxSeconds > 55) {
            throw new Error(`[GoogleSpeechProvider] Recording is approximately ${Math.round(approxSeconds)}s. ` +
    ...
    toProviderTranscript(response) {
        ...
        if (!authoritative) {
            throw new Error('[GoogleSpeechProvider] Recognition returned no transcribable speech. No clinical note ' +
                'can be generated from this recording.');
        }
        const words = authoritative.alternatives[0].words;
        ...
        if (speakerCount < 2) {
            ...
            console.warn('[GoogleSpeechProvider] Diarisation distinguished only one voice in this recording.');
        }
        return {
            providerName: this.name,
